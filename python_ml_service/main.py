...
@app.post('/query-rag')
async def api_query_rag(req: QueryRequest, request: Request):
    try:
        print(request.headers)
        # Extract Authorization header from incoming request
        auth_header = request.headers.get("Authorization")
        # print("auth_header:", auth_header)
        user_firebase_token = None
        if auth_header and auth_header.lower().startswith("bearer "):
            user_firebase_token = auth_header.split(" ", 1)[1]
        # Pass token to query_rag and downstream handlers
        ans = await query_rag(req.userId, req.query, req.history, user_firebase_token=user_firebase_token)
        return ans
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
