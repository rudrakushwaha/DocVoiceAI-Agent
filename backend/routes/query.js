...
router.post('/ask', async (req, res) => {
  ...
  const userFirebaseToken = req.headers['authorization'] || '';
  ...
  try {
    ...
    // 4. Call Python RAG with history, pass token in headers and body
    const pythonUrl = process.env.PYTHON_RAG_URL || 'http://localhost:8000/query-rag';
    const resp = await axios.post(
      pythonUrl,
      { userId: uid, query: message, history, user_firebase_token: userFirebaseToken },
      {
        timeout: 1000 * 60,
        headers: {
          Authorization: userFirebaseToken
        }
      }
    );
    ...
    // 5. Store assistant message with emotion and sources
    ...
    return res.json({ answer, sources, emotion: detectedEmotion, confidence, sessionId: session.sessionId });
  } catch (err) {
...
router.post('/voice', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    // Find or create session for voice queries
    let session;
    ...
    try {
      ...
      if (!session) {
        const { v4: uuidv4 } = require('uuid');
        sessionId = uuidv4();
        session = new Session({ sessionId, userId: uid, messages: [] });
        await session.save();
      }

      // Store user message
      session.messages.push({ role: 'user', content: transcribedText, timestamp: new Date() });
      if (session.messages.length > 32) session.messages = session.messages.slice(-32);
      ...
      if (transcribedText) {
        try {
          ...
          const ragResp = await axios.post(
            ragUrl,
            { userId: uid, query: transcribedText, user_firebase_token: userFirebaseToken },
            { headers: { Authorization: userFirebaseToken } }
          );
          if (ragResp && ragResp.data) {
            answer = ragResp.data.answer || transcribedText;
            sources = ragResp.data.sources || [];
            emotion = emotion || ragResp.data.emotion; // Use detected emotion from RAG
          }
        } catch (e) {
      ...
      // Store assistant message with emotion and sources
      session.messages.push({ 
        role: 'assistant', 
        content: answer, 
        emotion: emotion,
      ...
      return res.json({ text: transcribedText, answer, emotion, sources, sessionId: session.sessionId });
    } catch (sessionErr) {
      console.error('Session error for voice query:', sessionErr);
