...
export default function ChatWindow({messages, onSend, aiTyping, onStartVoice, voiceOutput, setVoiceOutput, renderPdfLink}){
        ...
        {messages.map((m,idx)=> {
          ...
          <div key={idx} style={{display:'flex',flexDirection:'column',gap:6,alignItems: m.role==='user' ? 'flex-end':'flex-start'}}>
            <div className={`message ${m.role==='user' ? 'user':'ai'}`}>
              ...
              {/* Render PDF download link if present */}
              {m.pdfUrl && (
                <div style={{ marginTop: 8 }}>
                  <a href={m.pdfUrl} target="_blank" rel="noopener noreferrer" className="pdf-download-link" style={{ color: 'white' }}>
                    📄 Download PDF
                  </a>
                </div>
              )}
              <div className="meta">
                ...
                <div style={{marginLeft:8}}>
                  {m.role==='ai' && <details><summary style={{cursor:'pointer',color:'#fff',fontSize:12}}>Show Sources</summary><ProvenanceViewer sources={m.sources}/></details>}
                </div>
              </div>
            </div>
          </div>
