    ...
    <div className="topbar" style={{position:'relative'}}>
      ...
      <div style={{display:'flex', alignItems:'center', position:'relative'}}>
        ...
        <div className="profile" onClick={()=>setOpen(v=>!v)} style={{cursor:'pointer'}}>
          <div style={{textAlign:'right', marginRight:8, color:'#fff'}}>
            <div style={{fontSize:13,fontWeight:700, color:'#fff'}}>{displayName}</div>
            <div style={{fontSize:12,color:'#fff'}}>{currentUser?.email}</div>
          </div>
          <div className="avatar">{(currentUser?.displayName||'U')[0].toUpperCase()}</div>
        </div>

        {open && (
          <div style={{position:'absolute', right:0, top:50, background:'var(--card)', border:'1px solid rgba(255,255,255,0.04)', borderRadius:8, boxShadow:'0 8px 30px rgba(0,0,0,0.6)', overflow:'hidden', zIndex:40}}>
            ...
            <button onClick={handleLogout} style={{display:'block', padding:'10px 16px', width:220, textAlign:'left', background:'transparent', border:'none', cursor:'pointer', color:'var(--text)'}}>Logout</button>
          </div>
        )}
      </div>
    </div>
