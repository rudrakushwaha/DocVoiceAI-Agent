...
export async function getAIResponse(message, emotion, sessionId) {
  ...
  try {
    ...
    const emotions = ['neutral', 'happy', 'sad', 'angry', 'frustrated', 'confused', 'excited'];
...
export function handleSpeak(text) {
  ...
  // Clean text before speaking - remove citations and noise
  const cleanText = text
    .replace(/\[\d+\]/g, '') // Remove [1], [2], etc.
    .replace(/\([^)]*\d+[^)]*\)/g, '') // Remove (source 1), etc.
  ...
  // Split long text into chunks (200 characters max)
  const chunkSize = 200;
  const chunks = [];
